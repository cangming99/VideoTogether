...
def compile(sourceSubDir, extension, rawFilename, subNameList: list, content):
    ...
    if r"{{{" not in content:
            ...
            for lan in languages:
                ...
                with open(rootPath.joinpath(languages[lan]), encoding="utf-8") as f:
                    ...
                    for key in strings:
                        ...
                        newContent = newContent.replace("{$" + key + "$}", strings[key])
                compile(
                    sourceSubDir, extension, rawFilename, newSubNameList, newContent
                )
            return
        ...
        if vtSourceConfig != None and "releaseTarget" in vtSourceConfig:
            resultPath = (
                Path(sourceSubDir)
                .joinpath(vtSourceConfig["releaseTarget"])
                .joinpath(resultFilename + extension)
            )
...
def build():
    ...
    for sourceSubDir, dirList, fileList in allSource:
        for sourceFile in fileList:
            ...
            compile(sourceSubDir, resultExtension, resultFilename, [], sourceContent)
...
if __name__ == "__main__":
    ...
    if not disable_network:
        os.system(
            "git clone https://github.com/VideoTogether/localvideo {}/source/local".format(
                rootPath
            )
        )
        ...
        os.system(
            "git clone https://github.com/VideoTogether/website_next {}/source/website".format(
                rootPath
            )
        )
        ...
        os.system(
            "git clone https://github.com/VideoTogether/setting {}/source/setting".format(
                rootPath
            )
        )
    ...
    def cp(src, dst):
        shutil.copyfile(rootPath.joinpath(src), rootPath.joinpath(dst))

    def mv(src, dst):
        shutil.move(rootPath.joinpath(src), rootPath.joinpath(dst))
    ...
    for lan in languages:
        if lan != "":
            mv(
                "release/local_video_player." + lan + ".html",
                "source/local/local_video_player." + lan + ".html",
            )
            mv(
                "release/local_videos." + lan + ".html",
                "source/local/local_videos." + lan + ".html",
            )
            mv(
                "release/local_page." + lan + ".js",
                "source/local/local_page." + lan + ".js",
            )
            cp("release/vt." + lan + ".user.js", "source/chrome/vt." + lan + ".user.js")
            cp(
                "release/vt." + lan + ".user.js",
                "source/firefox/vt." + lan + ".user.js",
            )
            cp(
                "release/vt." + lan + ".user.js",
                "source/safari/VideoTogether/Shared (Extension)/Resources/vt."
                + lan
                + ".user.js",
            )
    ...
    shutil.copyfile(
        rootPath.joinpath("release/extension.chrome.user.js"),
        rootPath.joinpath("source/chrome/extension.chrome.user.js"),
    )
    shutil.copyfile(
        rootPath.joinpath("release/extension.safari.user.js"),
        rootPath.joinpath(
            "source/safari/VideoTogether/Shared (Extension)/Resources/extension.safari.user.js"
        ),
    )
    shutil.copyfile(
        rootPath.joinpath("release/extension.firefox.user.js"),
        rootPath.joinpath("source/firefox/extension.firefox.user.js"),
    )

    shutil.copyfile(
        rootPath.joinpath("release/background.chrome.js"),
        rootPath.joinpath("source/chrome/background.chrome.js"),
    )
    shutil.copyfile(
        rootPath.joinpath("release/background.firefox.js"),
        rootPath.joinpath("source/firefox/background.firefox.js"),
    )
    shutil.copyfile(
        rootPath.joinpath("release/background.safari.js"),
        rootPath.joinpath(
            "source/safari/VideoTogether/Shared (Extension)/Resources/background.safari.js"
        ),
    )
