    ...
    const WS = {
        ...
        async onmessage(str) {
            ...
            if (data['method'] == 'send_txtmsg' && getEnableTextMessage()) {
                ...
                if (!isSelf) {
                    popupError("VideoTogether: 新しいメッセージ (<a id='changeVoiceBtn' style='color:inherit' href='#''>音声変更</a>)");
                }
...
<div id="videoTogetherFlyPannel" style="display: none;">
  <div id="videoTogetherHeader" class="vt-modal-header" style="flex-direction: column; align-items: stretch; gap: 4px;">
    ...
    <div style="display: flex; align-items: center; width: 100%;">
      ...
      <div style="flex: 1;"></div>
      <span id="videoTogetherStatusText" style="color: #666; font-size: 13px; flex-shrink: 0;"></span>
    </div>
  </div>
  ...
  <div class="vt-modal-content">
    ...
    <div class="vt-modal-body">
      <div id="mainPannel" class="content" style="display: flex; flex-direction: column; width: 100%; flex: 1; min-height: 0; gap: 8px; justify-content: center;">
        <div id="roomInputContainer" style="display: flex; flex-direction: column; gap: 12px; padding: 16px; width: 100%; background: #f8f9fa; border-radius: 12px; box-sizing: border-box;">
          <div style="display: flex; align-items: center; gap: 12px;">
            <span class="ellipsis" id="videoTogetherRoomNameLabel" style="width: 64px; flex-shrink: 0; font-size: 13px; color: #555; font-weight: 500;">ルーム</span>
            <input id="videoTogetherRoomNameInput" autocomplete="off" placeholder="ルーム名を入力してください" style="flex: 1; height: 38px; padding: 0 14px; font-size: 14px; border-radius: 10px; border: 1px solid #e0e0e0; background: #fff; outline: none; transition: border-color 0.2s, box-shadow 0.2s;">
          </div>
          <div style="display: flex; align-items: center; gap: 12px;">
            <span class="ellipsis" id="videoTogetherRoomPasswordLabel" style="width: 64px; flex-shrink: 0; font-size: 13px; color: #555; font-weight: 500; display: none;">パスワード</span>
            <input id="videoTogetherRoomPdIpt" autocomplete="off" placeholder="ホストのパスワードを入力" style="flex: 1; height: 38px; padding: 0 14px; font-size: 14px; border-radius: 10px; border: 1px solid #e0e0e0; background: #fff; outline: none; display: none; transition: border-color 0.2s, box-shadow 0.2s;">
          </div>
        </div>
      ...
      <div id="voicePannel" class="content" style="display: none; flex-direction: column; gap: 8px; width: 100%; flex: 1; min-height: 0;">
        <div id="videoVolumeCtrl" style="width: 100%;text-align: left;">
          <span style="display: inline-block;width: 100px;margin-left: 20px;">動画の音量</span>
        ...
        <div id="callVolumeCtrl" style="width: 100%;text-align: left;">
          <span style="display: inline-block;width: 100px;margin-left: 20px;">通話の音量</span>
          <div class="range-slider">
    ...
    <div class="vt-modal-footer">
      ...
      <div id="chatInputRow" style="display: flex; gap: 6px; width: 100%; align-items: center; padding: 0 6px; box-sizing: border-box;">
        <input id="textMessageInput" autocomplete="off" placeholder="テキストメッセージ" style="width: calc(100% - 82px); height: 34px; padding: 0 10px; font-size: 13px; border-radius: 6px; border: 1px solid #d9d9d9; box-sizing: border-box; outline: none;">
                ...
                this.chatHistoryBtn.onclick = async () => {
                    ...
                    modalContent.innerHTML = `
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
                            <h3 style="margin: 0; color: #1a1a1a;">チャット履歴管理</h3>
                    ...
                    if (roomIds.length === 0) {
                        roomList.innerHTML = `<div style="text-align: center; color: #999; padding: 30px;">チャット履歴がありません</div>`;
                        return;
                    }
                    ...
                    roomIds.forEach(roomId => {
                        ...
                        roomItem.querySelector(".export-btn").onclick = function() {
                            ...
                            popupError("クリップボードにエクスポートしました");
                        };

                        // 删除按钮
                        roomItem.querySelector(".delete-btn").onclick = function() {
                            if (confirm("この部屋のチャット履歴を削除してもよろしいですか？")) {
                                delete history[roomId];
